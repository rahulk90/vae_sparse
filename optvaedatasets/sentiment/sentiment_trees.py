"""
File modified from https://github.com/miyyer/dan
Same licence as in preprocess_imdb.py
"""
...
def buildWordMap():
    ...
    with open('./stanford/trees/wordMapAll.bin','w') as fid:
...
def loadTrees(dataSet='train', wmap=loadWordMap):
    ...
    """
    wordMap = wmap
    file = './stanford/trees/%s.txt'%dataSet
    print "Reading trees.."
    with open(file,'r') as fid:
        trees = [Tree(l) for l in fid.readlines()]
    for tree in trees:
...
def process_trees(wmap):
    for split in ['train', 'dev', 'test']:
        ...
        print len(trees)
        cPickle.dump(trees, open('./stanford/' + split + '_alltrees', 'wb'), protocol=cPickle.HIGHEST_PROTOCOL)
...
if __name__=='__main__':
    ...
    process_trees(wmap)
    train = cPickle.load(open('./stanford/train_alltrees', 'rb'))
    dev = cPickle.load(open('./stanford/dev_alltrees', 'rb'))
    test = cPickle.load(open('./stanford/test_alltrees', 'rb'))
    revMap = {}
    ...
    for sent, label in t_sents:
        c[label] += 1
    print c
    cPickle.dump(t_sents, open('./stanford/train-rootfine', 'wb'))
    ...
    for sent, label in dev_sents:
        c[label] += 1
    print c
    cPickle.dump(dev_sents, open('./stanford/dev-rootfine', 'wb'))
    ...
    for sent, label in test_sents:
        c[label] += 1
    print c
    cPickle.dump(test_sents, open('./stanford/test-rootfine', 'wb'))
